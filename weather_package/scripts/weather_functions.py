...
def min_max_day(api_key, city, country, day):
    ...
    final_url = str(url + 'city=' + city + '&country=' + country
                    + '&day' + str(day) + '&key=' + api_key)
    final_url = final_url.replace(' ', '%20')
    json_obj = urllib.request.urlopen(final_url)
    js = json.load(json_obj)

    w_info = str('In ' + str(city) + ' (' + str(country) + ') '
                 + 'the day ' + str(js['data'][day - 1]['datetime'])
                 + ' , min temp is ' + str(js['data'][day - 1]['min_temp'])
                 + ' ,max temp is ' + str(js['data'][day - 1]['max_temp'])
                 + ' with ' + str(js['data'][day - 1]['weather']
                 ['description'].lower()))
